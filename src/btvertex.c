...
PBTVNODE NewBTVNODE(INT NDim, PREAL pCoor) {
	...
	pBTVNode->pV = (PREAL) GetMem((SIZE) NDim, (SIZE) sizeof(REAL), "NewBTVNODE->pV");
...
/*---------------------------------------------------------------------------*/
PBTVNODE InsertBTV1(PBTVNODE root, INT NDim, PREAL pCoor, PBOOL ptaller, PBOOL pAdd) {
	COMPARE CompCoor;
	...
	if (root == NULL) {
		...
		*pAdd = True;
		...
		if (CompCoor != Equal) {
			if (CompCoor == Smaller) {
				root->pleft = InsertBTV1(root->pleft, NDim, pCoor, ptaller, pAdd);
				if (*ptaller)
					switch (root->Balance) {
					case LEFT:
				...
				root->pright = InsertBTV1(root->pright, NDim, pCoor, ptaller, pAdd);
				if (*ptaller)
					switch (root->Balance) {
					case LEFT:
						root->Balance = EQUAL;
...
VOID InsertBTV(PBTV pbtv, INT NDim, PREAL pCoor, PBOOL pAdd) {
	...
	*pAdd = False;
	pbtv->pFirstBTVNode = InsertBTV1(pbtv->pFirstBTVNode, NDim, pCoor, &taller, pAdd);
	if (*pAdd == True) {
		pbtv->NElem++;
