	...
	if (root == NULL) {
		...
		*ptaller = True;
		...
		if (pS->NSimplex == root->pS->NSimplex) {
			...
			exit(1);
		} else if (pS->NSimplex < root->pS->NSimplex) {
...
/*                                                                           */
/*---------------------------------------------------------------------------*/
PBTSNODE ExtractNodeEqual(PBTSNODE root, INT NDim, INT NSimplex, PBOOL pshorter, PPSIMPLEX ppS) {
	PBTSNODE Temp;
	...
	if (root == NULL) {
			...
			*ppS = root->pS;
			if (root->pleft == NULL && root->pright == NULL) {
				...
				*pshorter = True;
				...
				root->pright = ExtractNodeEqual(root->pright, NDim, NSimplex, pshorter, ppS);
				if (*pshorter)
					switch (root->Balance) {
					case LEFT:
		...
		} else if (NSimplex < root->pS->NSimplex) {
			root->pleft = ExtractNodeEqual(root->pleft, NDim, NSimplex, pshorter, ppS);
			if (*pshorter)
				switch (root->Balance) {
				case LEFT:
					root->Balance = EQUAL;
...
VOID ExtractEqualBTS(PBTS pbts, INT NDim, INT NSimplex, PPSIMPLEX ppS) {
	...
	pbts->pFirstBTSNode = ExtractNodeEqual(pbts->pFirstBTSNode, NDim, NSimplex, &shorter, ppS);
	pbts->NElem--;
}
...
/*                                                                           */
/*---------------------------------------------------------------------------*/
PBTSNODE ExtractSmallerNode(PBTSNODE root, INT NDim, PLLINT pNSimplex, PBOOL pshorter, PPSIMPLEX ppS) {
	PBTSNODE Temp;
	...
	if (root->pleft != NULL)
		root->pleft = ExtractSmallerNode(root->pleft, NDim, pNSimplex, pshorter, ppS);
	else {
		*ppS = root->pS;
...
VOID ExtractSmallerBTS(PBTS pbts, INT NDim, PPSIMPLEX ppS) {
	...
	pbts->pFirstBTSNode = ExtractSmallerNode(pbts->pFirstBTSNode, NDim, &NSimplex, &shorter, ppS);
	pbts->NElem--;
}
...
/*                                                                           */
/*---------------------------------------------------------------------------*/
PBTSNODE ExtractGreaterNode(PBTSNODE root, INT NDim, PLLINT pNSimplex, PBOOL pshorter, PPSIMPLEX ppS) {
	PBTSNODE Temp;
	...
	if (root->pright != NULL)
		root->pright = ExtractGreaterNode(root->pright, NDim, pNSimplex, pshorter, ppS);
	else {
		*ppS = root->pS;
		*pNSimplex = (*ppS)->NSimplex;
	}
...
VOID ExtractGreaterBTS(PBTS pbts, INT NDim, PPSIMPLEX ppS) {
	...
	pbts->pFirstBTSNode = ExtractGreaterNode(pbts->pFirstBTSNode, NDim, &NSimplex, &shorter, ppS);
	pbts->NElem--;
}
