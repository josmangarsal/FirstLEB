...
/*---------------------------------------------------------------------------*/
PSIMPLEX TerminationTest(PBTS pbts, PBTS pbtsEnd, PSIMPLEX pS, INT NDim, REAL Epsilon, PLLINT pNFinalSimplex, BOOL NoStoreFinalS, BOOL Draw,
		INT WWidth, PBTV pbtvVertices, PBTV pbtvGridPoints) {
	BOOL add;
	int i;
	for (i = 0; i < NDim; i++) {
...
PSIMPLEX GenInitialS(INT NDim) {
	...
	ppCoor = (PPREAL) GetMem2D((SIZE) NDim, (SIZE) NDim, (SIZE) sizeof(REAL), "GenInitalS:ppCoor");
...
int main(int argc, char *argv[]) {
	...
	NEvalSimplex++;
	pS = TerminationTest(pbts, pbtsEnd, pS, NDim, Epsilon, &NFinalSimplex, NoStoreFinalS, Draw, WWidth, pbtvVertices, pbtvGridPoints);
	...
	while (pS != NULL || pbts->NElem > 0) {
		...
		NEvalSimplex += 2;
		pS1 = TerminationTest(pbts, pbtsEnd, pS1, NDim, Epsilon, &NFinalSimplex, NoStoreFinalS, Draw, WWidth, pbtvVertices, pbtvGridPoints);
